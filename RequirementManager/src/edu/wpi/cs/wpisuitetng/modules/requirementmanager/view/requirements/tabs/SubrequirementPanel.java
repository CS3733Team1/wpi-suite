...
public class SubrequirementPanel extends JPanel implements
		RequirementSelectorListener, RequirementPanelListener {
	 ...
	 * @param vm
	 *            viewmode
	 * @param requirementBeingEdited
	 *            the current requirement being edited.
	 */
	public SubrequirementPanel(RequirementTabsPanel parentPanel, ViewMode vm,
			Requirement requirementBeingEdited) {
		this.setLayout(new BorderLayout());
		this.viewMode = vm;
		this.activeRequirement = requirementBeingEdited;
		existingReqSelector = new RequirementSelector(this, activeRequirement,
				RequirementSelectorMode.POSSIBLE_CHILDREN, false);
		...
		addNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				ViewEventController.getInstance().createChildRequirement(
						activeRequirement.getId());
			}
		});
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < selectedObjects.length; i++) {
					Requirement toBeRemoved = (Requirement) subRequirementTable
							.getValueAt(selectedObjects[i], 0);
					try {
					...
					UpdateRequirementController.getInstance()
							.updateRequirement(toBeRemoved);
					ViewEventController.getInstance()
							.refreshEditRequirementPanel(toBeRemoved);
					ViewEventController.getInstance()
							.refreshEditRequirementPanel(activeRequirement);
				}
	...
	private JTable buildTable() {
		subRequirementTable = new JTable() {
			/**
			 * Overrides the isCellEditable method to ensure no cells are
			 * editable.
			 * 
			 * @param row
			 *            row of OverviewTable cell is located
			 * @param col
			 *            column of OverviewTable cell is located
			 */
			@Override
		...
		subRequirementTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (subRequirementTable.getRowCount() > 0) {
					int mouseY = e.getY();
					Rectangle lastRow = subRequirementTable.getCellRect(
							subRequirementTable.getRowCount() - 1, 0, true);
					int lastRowY = lastRow.y + lastRow.height;

					if (mouseY > lastRowY) {
						subRequirementTable.getSelectionModel()
								.clearSelection();
						repaint();
					}
				}
				...
				if (e.getClickCount() == 2) {
					...
					if (selection.length != 1)
						return;
					Requirement toEdit = (Requirement) subRequirementTable
							.getValueAt(selection[0], 0);
					ViewEventController.getInstance().editRequirement(toEdit);
				}
			}
		});

		subRequirementTable.getSelectionModel().addListSelectionListener(
				new ListSelectionListener() {
					@Override
					public void valueChanged(ListSelectionEvent e) {
						removeButton.setEnabled(subRequirementTable
	...
	/**
	 * Populates the subrequirement table with the sub requirements of the
	...
	private void refreshTable() {
		...
		for (int i = 0; i < subRequirements.size(); i++) {
			...
			if (!req.isDeleted()) {
				tableModel.addRow(new Object[] { req, req.getDescription(),
						req.getPriority(), req.getEstimate() });
			}
		}
	}
	...
	/**
	 * Overrides the requirementSelected method to signal to refresh the table
	 * when the subrequirements are updated.
	 * 
	 * @param requirements
	 *            Object[]
