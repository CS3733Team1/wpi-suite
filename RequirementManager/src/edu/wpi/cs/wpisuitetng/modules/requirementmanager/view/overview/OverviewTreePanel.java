...
public class OverviewTreePanel extends JScrollPane implements MouseListener,
	...
	public void refresh() {
		...
		/* ConfigManager.getConfig().getProjectName() */"Iteration Tree"); // makes
																			// a
																			// starting
																			// node
		...
		for (int i = 0; i < iterations.size(); i++) {
			...
			DefaultMutableTreeNode newIterNode = new DefaultMutableTreeNode(
					iterations.get(i)); // make a new iteration node to add
			List<Requirement> requirements = sortRequirements(iterations.get(i)
					.getRequirements()); // gets the list of requirements that
											// is associated with the iteration
		...
		tree.getSelectionModel().setSelectionMode(
				TreeSelectionModel.SINGLE_TREE_SELECTION); // tell it that it
															// can only select
															// one thing at a
															// time
		tree.setToggleClickCount(0);

		tree.setCellRenderer(new CustomTreeCellRenderer()); // set to custom
															// cell renderer so
															// that icons make
															// sense
		...
		ViewEventController.getInstance().setOverviewTree(this); // update the
																	// ViewEventControler
																	// so it
																	// contains
																	// the right
	 ...
	 */
	private void addRequirementsToTree(List<Requirement> requirements,
			DefaultMutableTreeNode parentNode) {

		for (int j = 0; j < requirements.size(); j++) {
			DefaultMutableTreeNode newReqNode = new DefaultMutableTreeNode(
					requirements.get(j));
			List<Requirement> children = sortRequirements(requirements.get(j)
					.getChildren());
			// if((requirements.get(j).getParentID() != -1) &&
			// (parentNode.getUserObject() instanceof Iteration)){
			// System.out.println("breaking for " +
			// requirements.get(j).getName());
			// continue;
			// }
			if (children.size() > 0) {
	...
	public void mouseClicked(MouseEvent e) {
			...
			if (treePath != null) {
				...
				if (node != null) {
					if (node.getUserObject() instanceof Iteration) {
						ViewEventController.getInstance().editIteration(
								(Iteration) node.getUserObject());
					}
					if (node.getUserObject() instanceof Requirement) {
						ViewEventController.getInstance().editRequirement(
								(Requirement) node.getUserObject());
					}
				}
			}
		}
		 ...
		 * tree.setSelectionRow(row); String label = "popup: "; JPopupMenu popup
		 * = new JPopupMenu(); popup.add(new JMenuItem(label)); popup.show(tree,
		 * x, y); }
		 */
	}
	...
	public void valueChanged(TreeSelectionEvent e) {
		...
		if (node != null) {
			if (node.getUserObject() instanceof Iteration) {
				ViewEventController.getInstance().getIterationOverview()
						.highlight((Iteration) node.getUserObject());
			} else {
				ViewEventController.getInstance().getIterationOverview()
						.highlight(IterationModel.getInstance().getBacklog());
			}
		}
	}
	...
	/**
	 * @param list
	 *            the list of iterations to be sorted
...
/**
 * @author Kevin sorts the Iterations by date
 * 
 */
...
/**
 * @author Kevin sorts Requirements by name
 * 
 */
