...
public class EditRequirementPanelTest {
	...
	public void setUp() throws Exception {
		...
		String[] columnNames = { "ID", "Name", "Release #", "Iteration",
				"Type", "Status", "Priority", "Estimate" };
		Object[][] data = {};
	...
	public void defaultEnabilityAndField() {
		...
		assertEquals(true, testEdit.getInfoPanel().getBoxDescription()
				.isEnabled());
		assertEquals(false, testEdit.getInfoPanel().getBoxIteration()
				.isEnabled());
		assertEquals(true, testEdit.getInfoPanel().getDropdownType()
				.isEnabled());
		assertEquals(true, testEdit.getInfoPanel().getDropdownStatus()
				.isEnabled());
		...
		assertEquals(false, testEdit.getButtonPanel().getButtonClear()
				.isEnabled());
		assertEquals(true, testEdit.getButtonPanel().getButtonCancel()
				.isEnabled());
		assertEquals(true, testEdit.getButtonPanel().getButtonDelete()
				.isEnabled());

		// Check for default values in each field
		assertEquals(testName, testEdit.getInfoPanel().getBoxName().getText());
		assertEquals(testDescription, testEdit.getInfoPanel()
				.getBoxDescription().getText());
		assertEquals(testRelease, testEdit.getInfoPanel().getBoxReleaseNum()
				.getText());
		assertEquals(RequirementStatus.NEW, testEdit.getInfoPanel()
				.getDropdownStatus().getSelectedItem());
		assertEquals("0", testEdit.getInfoPanel().getBoxEstimate().getText());
		assertEquals(RequirementType.EPIC, testEdit.getInfoPanel()
				.getDropdownType().getSelectedItem());
	...
	public void errorFieldTest() {
		...
		assertEquals(errorMessageNoninterger, testEdit.getInfoPanel()
				.getErrorEstimate().getText());
		assertEquals(errorMessageNoMore100, testEdit.getInfoPanel()
				.getErrorName().getText());
		...
		assertEquals(errorMessageNoninterger, testEdit.getInfoPanel()
				.getErrorEstimate().getText());
		assertEquals(errorMessageRequiredDescription, testEdit.getInfoPanel()
				.getErrorDescription().getText());
	...
	public void errorRequiredFieldTest() {
		...
		RequirementPanel testEdit = new RequirementPanel(testRequirement);
		// a field is added correctly but both name and description are filled
		// with blanks
		testEdit.getInfoPanel().getBoxEstimate().setText("-134");
		...
		assertEquals(false, testEdit.getButtonPanel().getButtonOK().isEnabled());
		assertEquals(true, testEdit.getButtonPanel().getButtonClear()
				.isEnabled());
		assertEquals(true, testEdit.getButtonPanel().getButtonCancel()
				.isEnabled());
		...
		// can't create because no name/description, but a field has been
		// changed
		assertEquals(false, testEdit.getButtonPanel().getButtonOK().isEnabled());
		assertEquals(true, testEdit.getButtonPanel().getButtonClear()
				.isEnabled());
	...
	public void undoChangeButtonTest() {
		...
		testEdit.getInfoPanel().getBoxIteration()
				.setSelectedItem("Iteration test");
		testEdit.getInfoPanel().getDropdownType()
				.setSelectedItem(RequirementType.SCENARIO);
		testEdit.getInfoPanel().getDropdownStatus()
				.setSelectedItem(RequirementStatus.INPROGRESS);
	 ...
	 */
	/*
	 * @Test public void updateButtonTest2() { Requirement testRequirement = new
	 * Requirement(); String testName = "test: Name"; String testDescription =
	 * "test: Description"; String testRelease = "1.0.2"; int testEstimate = 0;
	 * //int testActualEffort = 0;
	 * 
	 * String updateTestName = "Update test: Name"; String updateTestDescription
	 * = "Update test: Description"; String updateTestRelease = "1.0.3"; int
	 * updateTestEstimate = 1;
	 * 
	 * testRequirement.setName(testName);
	 * testRequirement.setDescription(testDescription);
	 * testRequirement.setRelease(testRelease);
	 ...
	 * testEdit.getInfoPanel
	 * ().getBoxDescription().setText(updateTestDescription);
	 * testEdit.getInfoPanel
	 * ().getDropdownType().setSelectedItem(RequirementType.THEME);
	 * testEdit.getInfoPanel
	 * ().getDropdownStatus().setSelectedItem(RequirementStatus.INPROGRESS);
	 * testEdit.getInfoPanel().getPriorityHigh().doClick();
	 * testEdit.getInfoPanel().getBoxEstimate().setText("4");
	 * testEdit.getInfoPanel().getBoxIteration().addItem("Iteration test");
	 * testEdit
	 * .getInfoPanel().getBoxIteration().setSelectedItem("Iteration test");
	 * testEdit.getInfoPanel().update();
	 * 
	 * // check the result
	 * assertEquals(updateTestName,testEdit.getDisplayRequirement().getName());
	 * assertEquals
	 * (updateTestDescription,testEdit.getDisplayRequirement().getDescription
	 * ());
